...
async def espnow(device_id, peer, storage, log=None):
    enow = AIOESPNow()
    enow.active(True)

    try:
        ch = storage.get("channel")
        if ch:
            if log:
                log.debug(f"load channel from RTC memory {ch}")
            enow.add_peer(peer)
            channel(ch)
            return enow

        ch = await find_channel(device_id, peer, log)
        if log:
            log.debug(f"found channel {ch}")
        storage.set("channel", ch)

    except Exception as e:
        if log:
            log.error(f"espnow error: {e}")

    return enow


async def find_channel(device_id, peer, log=None):
    enow = AIOESPNow()
    if peer:
        enow.add_peer(peer)
    ...
    for ch in range(1, 14):
        if log:
            log.debug(f"trying channel {channel}")
        channel(ch)
        ...
        try:
            ...
            if await enow.asend(peer, ujson.dumps(msg).encode('utf-8')):  # type: ignore
                found.append(ch)
        except Exception as e:
            if log:
                log.error(f"espnow error: {e}")
    ...
    if log:
        log.debug(f"found channels: {found}")
    ...
    ch = found[index]
    if log:
        log.debug(f"setting wifi channel to {channel}")
    channel(ch)
